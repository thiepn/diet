...
async function runReleaseCheck(showFeedback = true) {
    ...
    if (cloud.client && cloud.user && cloudConfig.activated) {
      if (!isOnline()) qaReport.cloud = { level: 'warn', detail: 'Offline — cloud schema not checked' };
        ...
        try {
          ...
          const versionOk = Number(data?.schema_version) >= DB_SCHEMA_VERSION;
          const integrity = data?.integrity || {};
          const orphanTotal = Object.values(integrity).reduce((a,v) => a + (Number(v) || 0), 0);
          qaReport.cloud = { level: versionOk && orphanTotal === 0 ? 'pass' : 'fail', detail: versionOk ? (orphanTotal ? `${orphanTotal} cloud integrity issue${orphanTotal === 1 ? '' : 's'}` : `Schema v${data.schema_version} · RLS/RPC reachable`) : `Schema v${data?.schema_version ?? '?'} — update the cloud schema` };
        } catch (e) {
          qaReport.cloud = { level: 'fail', detail: `Health check failed: ${e.message || e}. Install/update the Diet Copilot Supabase schema.` };
        }
      }
    ...
    if (currentView === 'settings') renderSettings();
    if (showFeedback) showToast(qaReport.local.failures || qaReport.cloud?.level === 'fail' ? 'Health check found issues' : 'Health check complete');
  }
...
function restoreMigrationBackup() {
    ...
    if (!backup?.state) { showToast('No pre-V1 migration backup found'); return; }
    ...
    if (!prepareDestructiveAction('before migration rollback')) return;
    state = normalizeState(backup.state); state.syncQueue = []; clearCloudBaselines(); cloudConfig.activated = false; unsubscribeRealtime(); saveCloudConfig(); saveState(); render(); showToast('Pre-V1 local backup restored');
  }
