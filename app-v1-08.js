...
function cleanEditorItems(items, fallbackName, fallback = {}) {
    return items.filter(i => String(i.name || '').trim() || Number(i.calories || 0) || Number(i.protein || 0)).map(i => normalizeItem({ ...i, name: String(i.name || fallbackName || 'Food').trim(), confidence: i.confidence || fallback.confidence, source: i.source || fallback.source }));
  }
