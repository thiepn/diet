...
for(const file of ['diet-app.js','diet.css','index.html','sw.js','.well-known/thiepn-app.json'])assert.ok(fs.existsSync(file),`Missing ${file}`);

const js=fs.readFileSync('diet-app.js','utf8');
const css=fs.readFileSync('diet.css','utf8');
...
for(const required of ['renderInsightsV66','renderHistoryV66','renderTodayV67','V67_WEB_RELEASE','Logged nutrition always counts','window.DietOperations','window.DietRelease'])assert.ok(js.includes(required),`Production JS missing ${required}`);
...
assert.ok(sizeKB<450,`Production JS bundle exceeds 450 KB budget: ${sizeKB.toFixed(1)} KB`);
const cssKB=Buffer.byteLength(css)/1024;
assert.ok(cssKB<180,`Production CSS bundle exceeds 180 KB budget: ${cssKB.toFixed(1)} KB`);
