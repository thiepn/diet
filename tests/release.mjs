...
assert.match(html,/diet\.css\?v=1\.0\.2-static4/);
assert.match(html,/diet-app\.js\?v=1\.0\.2-static4/);
...
assert.ok(sw.includes("const CACHE='diet-copilot-web-v1.0.2-static4'"),'Wrong service-worker generation');
assert.ok(sw.includes("'./diet-app.js?v=1.0.2-static4'"),'Service worker missing stable JS bundle');
assert.ok(sw.includes("'./diet.css?v=1.0.2-static4'"),'Service worker missing stable CSS bundle');
...
console.log(`Diet Copilot Web 1.0.2 static4 runtime certified: ${sizeKB.toFixed(1)} KB JS, ${cssKB.toFixed(1)} KB CSS.`);
