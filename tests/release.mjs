...
for(const file of ['diet-app.js','diet.css','.v68-build/diet-app.js','.v68-build/diet.css','index.html','sw.js','.well-known/thiepn-app.json','src/release.js'])assert.ok(fs.existsSync(file),`Missing ${file}`);
...
for(const required of ['renderInsightsV66','renderHistoryV66','renderTodayV68','DIET_WEB_RELEASE = \'1.0.0\'','DIET_RELEASE_CHANNEL = \'stable\'','Logged nutrition always counts','window.DietOperations','window.DietRelease'])assert.ok(js.includes(required),`Expanded production JS missing ${required}`);
...
assert.equal(manifest.release,'V6.8');
...
console.log(`Diet Copilot Web 1.0.0 release certified: ${sizeKB.toFixed(1)} KB JS, ${cssKB.toFixed(1)} KB CSS.`);
